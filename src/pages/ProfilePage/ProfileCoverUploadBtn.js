...
const ProfileCoverUploadBtn = ({ id, target }) => {
  ...
  const onUploadClick = async () => {
    ...
    try {
      ...
      const coverFileRef = storage.ref(`/profiles/${id}`).child('topBanner');
      const uploadCoverResult = await coverFileRef.put(blob, {
        cacheControl: `public, max-age=${3600 * 24 * 3}`,
      });
      const downloadUrl = uploadCoverResult.ref.getDownloadURL();
      const profileCoverRef = database.ref(`/profiles/${id}/topBanner`);
  ...
  return (
    <div className="text-center">
      <label
        ...
        style={{ position: 'absolute', left: '24vw' }}
      >
        <h5>
      ...
      <Modal show={isOpen} onHide={close}>
        <Modal.Header>
          <Modal.Title>Upload New Profile Cover</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default memo(ProfileCoverUploadBtn);
